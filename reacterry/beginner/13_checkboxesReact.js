...
const checkSelectedItem = (id) =>{
  setCheckboxes(checkboxes.map((checkbox) => (checkbox.id === id ? { ...checkbox, checked: !checkbox.checked? true : false }:checkbox)));
}
const checkAllItems = () =>{
      ...
      {checkboxes.map((checkbox, index) => (
        <CheckboxLabel key={checkbox.id}>
          <input
            ...
            onClick={()=>{checkSelectedItem(checkbox.id)}}
          />
          {checkbox.label}
        </CheckboxLabel>
      ))}
    </CheckboxContainer>
    <SelectAllButton onClick={checkAllItems} data-testid="button">Select All</SelectAllButton>
  </Container>
);
};
