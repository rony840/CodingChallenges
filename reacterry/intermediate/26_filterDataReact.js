...
const LabelFilter = () => {
  ...
  const selectedCat = (key) => {
    let active = [...category]
    if(active.includes(key)){
      let index = active.findIndex((k)=> k == key? key : null )
  ...
  return (
    <Wrapper>
      <div data-testid='labels-wrapper-id' className='label-container'>
        {
          ...
          (<div 
            ...
            className='label'
            key={animalClass}
            style={(category.includes(animalClass) == true)?{backgroundColor:'#333', color:'#fff'}:{}}
            onClick={()=>{selectedCat(animalClass)}}
          >
            {animalClass}
          </div>)
         ))
      ...
      <div data-testid='tile-container-id' className='tile-container'>
        { (category.length > 0) ?
          animalData.filter((animal)=>category.includes(animal.class)).map((animal) => (
